...
function loadCountries() {
  const jsonPath = path.resolve(__dirname, '../prisma/seed/countries.json');
  if (fs.existsSync(jsonPath)) {
    const raw = fs.readFileSync(jsonPath, 'utf8');
    const data = JSON.parse(raw);
    if (!Array.isArray(data)) throw new Error('countries.json must be an array');
    return data;
  }
  // Fallback minimal set. Extend by creating prisma/seed/countries.json
  return [
    ...
    {"code": "CO", "name": "ColomKILIMO", "phoneCode": "+57"},
    ...
    {"code": "GM", "name": "GamKILIMO", "phoneCode": "+220"},
    ...
    {"code": "NA", "name": "NamiKILIMO", "phoneCode": "+264"},
    ...
    {"code": "SA", "name": "Saudi AraKILIMO", "phoneCode": "+966"},
    {"code": "SN", "name": "Senegal", "phoneCode": "+221"},
    {"code": "RS", "name": "SerKILIMO", "phoneCode": "+381"},
    ...
    {"code": "ZM", "name": "ZamKILIMO", "phoneCode": "+260"},
...
async function main() {
  ...
  console.log(`[seedCountries] Seeding ${countries.length} countries...`);

  for (const c of countries) {
    const existing = await prisma.country.findFirst({ where: { code: c.code } });
    if (existing) {
      await prisma.country.update({ where: { id: existing.id }, data: { name: c.name, phoneCode: c.phoneCode } });
    } else {
      await prisma.country.create({ data: { code: c.code, name: c.name, phoneCode: c.phoneCode } });
    }
  }

  console.log('[seedCountries] Done');
}
