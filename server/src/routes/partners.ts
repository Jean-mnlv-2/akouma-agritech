...
// Admin list (all)
partnersRouter.get('/admin', authRequired, moduleAccess('partners'), async (_req: Request, res: Response) => {
  const items = await prisma.partner.findMany({ orderBy: [{ order: 'asc' }, { id: 'desc' }] });
  res.json({ data: items });
});

partnersRouter.post('/', authRequired, moduleAccess('partners'), async (req: Request, res: Response) => {
...
partnersRouter.put('/:id', authRequired, moduleAccess('partners'), async (req: Request, res: Response) => {
